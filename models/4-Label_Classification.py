...
#
# # 4-Label classification: Exclude "Supportive" class
#
...
w2v_file = {'file': '../data/english_conceptnet.txt', 'is_binary': False, 'limit': None }

op_file = '../out/Result_4-Label_Classification.tsv'
    ...
    if ip_file:
        with open(ip_file) as csv_file:
            ...
            for row in csv_reader:
                ...
                # if the class=row[2] not to be excluded, then proceed
                if not row[2] == exclude_clss:
...
def read_data(ip_file):
    ...
    # Read the input file
    x_data, y_data = read_ip_file(ip_file, exclude_clss="Supportive")
